...
class Rack::Attack
  ...
  throttle('throttle_authenticated_api', limit: 10000 , period: 5.minutes) do |req|
    req.authenticated_user_id if req.api_request?
  end

  throttle('throttle_per_token_api', limit: 10000, period: 5.minutes) do |req|
    req.authenticated_token_id if req.api_request?
  end

  throttle('throttle_unauthenticated_api', limit: 10000, period: 5.minutes) do |req|
    req.throttleable_remote_ip if req.api_request? && req.unauthenticated?
  end

  throttle('throttle_api_media', limit: 10000, period: 30.minutes) do |req|
    req.authenticated_user_id if req.post? && req.path.match?(%r{\A/api/v\d+/media\z}i)
  end

  throttle('throttle_media_proxy', limit: 10000, period: 10.minutes) do |req|
    req.throttleable_remote_ip if req.path.start_with?('/media_proxy')
  end

  throttle('throttle_api_sign_up', limit: 10000, period: 30.minutes) do |req|
    req.throttleable_remote_ip if req.post? && req.path == '/api/v1/accounts'
  end

  throttle('throttle_authenticated_paging', limit: 10000, period: 15.minutes) do |req|
    req.authenticated_user_id if req.paging_request?
  end

  throttle('throttle_unauthenticated_paging', limit: 10000, period: 15.minutes) do |req|
  ...
  throttle('throttle_api_delete', limit: 10000, period: 30.minutes) do |req|
    req.authenticated_user_id if (req.post? && req.path.match?(API_DELETE_REBLOG_REGEX)) || (req.delete? && req.path.match?(API_DELETE_STATUS_REGEX))
  end

  throttle('throttle_oauth_application_registrations/ip', limit: 10000, period: 10.minutes) do |req|
    req.throttleable_remote_ip if req.post? && req.path == '/api/v1/apps'
  end

  throttle('throttle_sign_up_attempts/ip', limit: 10000, period: 5.minutes) do |req|
    req.throttleable_remote_ip if req.post? && req.path_matches?('/auth')
  end

  throttle('throttle_password_resets/ip', limit: 10000, period: 5.minutes) do |req|
    req.throttleable_remote_ip if req.post? && req.path_matches?('/auth/password')
  end

  throttle('throttle_password_resets/email', limit: 10000, period: 30.minutes) do |req|
    req.params.dig('user', 'email').presence if req.post? && req.path_matches?('/auth/password')
  end

  throttle('throttle_email_confirmations/ip', limit: 10000, period: 5.minutes) do |req|
  ...
  throttle('throttle_email_confirmations/email', limit: 10000, period: 30.minutes) do |req|
  ...
  throttle('throttle_auth_setup/email', limit: 10000, period: 10.minutes) do |req|
    req.params.dig('user', 'email').presence if (req.put? || req.patch?) && req.path_matches?('/auth/setup')
  end

  throttle('throttle_auth_setup/account', limit: 10000, period: 10.minutes) do |req|
    req.warden_user_id if (req.put? || req.patch?) && req.path_matches?('/auth/setup')
  end

  throttle('throttle_login_attempts/ip', limit: 10000, period: 5.minutes) do |req|
    req.throttleable_remote_ip if req.post? && req.path_matches?('/auth/sign_in')
  end

  throttle('throttle_login_attempts/email', limit: 10000, period: 1.hour) do |req|
    req.session[:attempt_user_id] || req.params.dig('user', 'email').presence if req.post? && req.path_matches?('/auth/sign_in')
  end

  throttle('throttle_password_change/account', limit: 10000, period: 10.minutes) do |req|
